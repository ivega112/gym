...
import { getExpiredSubscriptions } from "@/app/actions/subscriptions";
...
export default function ExpiredPage() {
  ...
  const loadData = async () => {
    setIsLoading(true);
    const data = await getExpiredSubscriptions();
  ...
  const openWhatsApp = (phone: string, memberName: string, endDate: Date, gymName: string) => {
    const text = `مرحباً ${memberName} 👋\n\nنود إعلامك بأن اشتراكك في ${gymName} قد انتهى بتاريخ ${new Date(endDate).toLocaleDateString("ar-SA")}.\n\nيمكنك التواصل معنا لتجديد اشتراكك.\n\nمع تحيات إدارة ${gymName}`;
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">الاشتراكات المنتهية</h1>
        <p className="text-gray-500">الاشتراكات التي انتهت ولم يتم تجديدها بعد.</p>
      </div>
            ...
            <Table dir="rtl">
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">أيام الانتهاء</TableHead>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  <TableRow><TableCell colSpan={5} className="text-center py-8">لا توجد اشتراكات منتهية.</TableCell></TableRow>
                ) : (
                  filteredSubscriptions.map((sub) => (
                    <TableRow key={sub.id}>
                      ...
                      <TableCell>
                        <span className="text-red-600 font-bold">{Math.abs(sub.remainingDays)} يوم</span>
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <Button variant="default" size="sm" className="bg-red-600 hover:bg-red-700" onClick={() => openWhatsApp(sub.phone, sub.memberName, sub.endDate, sub.gymName)}>
